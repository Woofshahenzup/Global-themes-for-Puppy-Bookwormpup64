...
tar -xf $APPDIR/tar-themes/Nordic.tar.xz -C /usr/share/themes &
#tar -xf $APPDIR/tar-themes/Nordic-bluish-accent.tar.gz -C /usr/share/themes &
tar -xf $APPDIR/tar-themes/Nordzy-turquoise-dark.tar.gz -C /usr/share/icons/ &
cp -r $APPDIR/var / &&
cp -r $APPDIR/root / &&
cp -r $APPDIR/usr / &&
cp -r $APPDIR/etc / &&
...
sleep 01
ln -sf /root/.icons/panel-icons/nordic/*.* /usr/share/pixmaps/puppy &&
