...
getIconTheme()
{
 local Icon_theme=$(cat "/etc/desktop_icon_theme")
 printf "$Icon_theme"
}
##==============================================================================
##
getGtkTheme()
{
 local gtk_theme=$(grep '^include "/usr/share/themes/' /root/.gtkrc-2.0 | cut -f 5 -d '/')
 printf "$gtk_theme"
}
##==============================================================================
##
getWindowManager()
{
 local window_manager=$(wmctrl -m | grep "Name:" | awk '{print $2}')
 printf "$window_manager"
}
...
##	ONE LINERS
##==============================================================================
printInfoWindowManager(){ printInfoLine "  WM" "  $(getWindowManager)" ; }
printInfoGtkTheme()     { printInfoLine "  Gtk Theme" "  $(getGtkTheme)" ; }
printInfoIconTheme()    { printInfoLine "  Icon Theme" "  $(getIconTheme)" ; }
...
printInfoDate()         { printInfoLine "󰸗  Date " "   $(getDate)" ; }
...
printInfoCPU()          { printInfoLine "󰻠  CPU" "   $(getNameCPU)" ; }
printInfoCPULoad()      { printInfoLine "Sys load" "$(getCPULoad)" ; }
printInfoLocalIPv4()    { printInfoLine "󰩠  Local IPv4" "   $(getLocalIPv4)" ; }
...
printInfoColorpaletteSmall()
{
	...
	"$(formatText "$char" -c 167 )"\
	"$(formatText "$char" -c red )"\
	"$(formatText "$char" -c green )"\
	"$(formatText "$char" -c yellow )"\
	"$(formatText "$char" -c blue )"\
	"$(formatText "$char" -c magenta )"\
	"$(formatText "$char" -c cyan )"\
	"$(formatText "$char" -c 24 )")
	printInfoLine "  Palette" "$palette"
}
##------------------------------------------------------------------------------
##
printInfoColorpaletteFancy()
{
	...
	printInfoLine "Palette" "$palette_bot"
}
##------------------------------------------------------------------------------
##
printInfoCPUTemp()
...
printStatusInfo()
{
	...
	statusSwitch()
	{
		...
		## 	INFO (TEXT ONLY)
		##	NAME            FUNCTION
		    WINDOW_MANAGER) printInfoWindowManager;;
		    GTK_THEME)      printInfoGtkTheme;;
		    ICON_THEME)     printInfoIconTheme;;
			OS)             printInfoOS;;
